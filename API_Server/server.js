...
app.get("/time", (req, res) => {
  const date = new Date();
  pushData(getDB("Patients", "medication_log"), { timestamp: date }).then(
    (resp) => {
      res.send(resp);
    }
  );
...
app.post("/drugs", (req, res) => {
  ...
  getData(getDB("DrugProducts", "Drugs"), {
    DrugName: { $regex: drugName.toUpperCase() },
  }).then((drugDB) => {
    console.log(drugDB);
    res.send(drugDB);
  });
...
app.post("/supple", (req, res) => {
  ...
  getData(getDB("DrugProducts", "Supplements"), {
    "Product Name": {
      $regex: Supplement.charAt(0).toUpperCase() + Supplement.slice(1),
    },
  }).then((drugDB) => {
    console.log(drugDB);
    res.send(drugDB);
